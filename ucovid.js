...
ucovid.genomes=async (url=ucovid.genomeListURL)=>{ // list with results will be stored at ucovid.genomes.list
...
ucovid.updateGenomes=async(url=ucovid.genomeListURL)=>{
    if(typeof(localforage)=='undefined'){
        console.log('localforage persistency not available in this machine')
    }else{
        let txt = await(await fetch(url)).text()
        ucovid.genomes.list = JSON.parse(`[${txt.replace(/[\n\r]+/g,',').slice(0,-1)}]`)
        localforage.setItem('ucovid.genomes.list',ucovid.genomes.list)
